...
class RichCurses:
    ...
    def add_rich(self, win, y: int, x: int, markup: str, max_width: int | None = None) -> int:
        ...
        for span in text.spans:
            ...
            # Robustly parse style: may be str or Style
...
# If your parser uses tqdm, avoid MP lock issues in TUI:
...
def build_choices(majors: list[tuple[str, list[str]]]) -> tuple[list[tuple[str, str | None]], list[int]]:
    ...
    rows: list[(major_label, subtopic_or_None)] where:
      - subtopic == HEADER_SENTINEL → non-selectable header
      - subtopic == None            → 'only major topic' selectable
      - (None, None)                → global 'Could not identify topic' selectable
    selectable_idx: indices in rows that are selectable (exclude headers)
    """
    ...
    # single global escape at the very end (guard against duplicates)
...
def draw_middle(
    ...
    for i, row_idx in enumerate(range(scroll_top, end)):
        ...
        # Draw with selection background if selected
        if is_selected:
...
def label_loop(stdscr, articles, majors, defs: dict[str, str], csv_path: Path):
    ...
    for uri, art in articles:
        ...
        # --- now size panes so the entire title+abstract is visible ---
        # Reserve a minimal middle list height; shrink it if needed.
        min_mid_h = 3  # keep the list tiny if the text is huge
        max_top_h = max(1, H - prompt_h - min_mid_h)
        # Fit all top lines if possible, else take as much as we can.
        ...
        # Selection state for middle list (start at first selectable row)
        sel_pos = 0
        ...
        while True:
            ...
            # Keep selection visible; also show section header when on its first item
            ...
            # If the row above selection is a header, reveal it
            prev_idx = sel_idx - 1
            if 0 <= prev_idx < len(rows) and rows[prev_idx][1] == HEADER_SENTINEL:
                if prev_idx < desired_top:
                    desired_top = prev_idx

            # When on the very first selectable row, show from the top
            if sel_pos == 0:
                desired_top = 0
...
def main():
    ...
    # Load and filter articles (English only here)
    ...
    # (Comment if not desired) get only articles with non-empty abstracts
